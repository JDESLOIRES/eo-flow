...
class TCNModel(BaseCustomTempnetsModel):
    ...
    class TCNModelSchema(BaseTempnetsModel._Schema):
        ...
        kernel_size = fields.Int(
            missing=2, description="Size of the convolution kernels."
        )
        nb_filters = fields.Int(
            missing=64, description="Number of convolutional filters."
        )
        nb_conv_stacks = fields.Int(missing=1)
        dilations = fields.List(
            fields.Int,
            missing=[1, 2, 4, 8, 16, 32],
            description="Size of dilations used in the " "covolutional tf.keras.layers",
        )
        padding = fields.String(
            missing="CAUSAL",
            validate=OneOf(["CAUSAL", "SAME"]),
            description="Padding type used in convolutions.",
        )
        use_skip_connections = fields.Bool(
            missing=True, description="Flag to whether to use skip connections."
        )
        return_sequences = fields.Bool(
            missing=False, description="Flag to whether return sequences or not."
        )
        activation = fields.Str(
            missing="linear", description="Activation function used in final filters."
        )
        ...
        kernel_regularizer = fields.Float(
            missing=0, description="L2 regularization parameter."
        )
    ...
    def build(self, inputs_shape):
        ...
        for _ in range(self.config.nb_conv_stacks):
            for d in self.config.dilations:
                net, skip_out = ResidualBlock(
                    ...
                    name=f"residual_block_{len(residual_blocks)}",
                )(net)
        ...
        output_slice_index = (
            int(net.shape.as_list()[1] / 2)
            if self.config.padding.lower() == "same"
            else -1
        )
...
class MultiBranchCNN(BaseCustomTempnetsModel):
    ...
    def _cnn_layer(self, net, kernel_size, filters, first=False):
        ...
        if self.config.pooling:
            layer = tf.keras.layers.AveragePooling1D(
    ...
    def build(self, inputs_shape, lambda_=1.0):
        ...
        if self.config.multioutput or self.config.loss in ["gaussian", "laplacian"]:
            ...
            self.net = tf.keras.Model(
                inputs=x, outputs=[output, output_sigma, embedding]
            )
        else:
...
class BayesTempCNNModel(BaseCustomTempnetsModel):
    ...
    class BayesTempCNNModel(BaseCustomTempnetsModel._Schema):
        ...
        nb_fc_stacks = fields.Int(
            missing=1, description="Number of fully connected tf.keras.layers."
        )
        fc_activation = fields.Str(
            missing="relu",
            description="Activation function used in final FC tf.keras.layers.",
        ...
        residual_block = fields.Bool(missing=False, description="Add residual block")
        activity_regularizer = fields.Float(
    ...
    def _cnn_layer(self, net, i=0, first=False):
        ...
        layer = tfp.layers.Convolution1DReparameterization(
            filters=filters,
            kernel_size=kernel_size,
            strides=n_strides,
            activation=self.config.activation,
    ...
    def _fcn_layer(self, net, i=0):
        ...
        layer_fcn = tfp.layers.DenseReparameterization(
            units=nb_neurons,
            activation=self.config.activation,
            activity_regularizer=tf.keras.regularizers.l2(
                self.config.activity_regularizer
    ...
    def build(self, inputs_shape):
        ...
        output = tfp.layers.DenseReparameterization(
            units=self.config.n_classes,
            activity_regularizer=tf.keras.regularizers.l2(
                self.config.activity_regularizer
            ),
        ...
        if self.config.loss in ["gaussian", "laplacian"]:
            ...
            output_sigma = tfp.layers.DenseReparameterization(
                units=self.config.n_classes,
                activity_regularizer=tf.keras.regularizers.l2(
                    self.config.activity_regularizer
                ),
...
class HistogramCNNModel(BaseCustomTempnetsModel):
    ...
    class HistogramCNNModel(BaseCustomTempnetsModel._Schema):
        ...
        kernel_size = fields.List(
            fields.Int, missing=[3, 3], description="Size of the convolution kernels."
        )
        nb_conv_filters = fields.Int(
            missing=16, description="Number of convolutional filters."
        )
        ...
        n_strides = fields.List(
            fields.Int, missing=[1, 1], description="Value of convolutional strides."
        )
        nb_fc_neurons = fields.Int(
            missing=256, description="Number of Fully Connect neurons."
        )
...
class InceptionCNN(BaseCustomTempnetsModel):
    ...
    class InceptionCNN(BaseCustomTempnetsModel._Schema):
        ...
        nb_conv_filters = fields.Int(
            missing=32, description="Number of convolutional filters."
        )
        nb_conv_stacks = fields.Int(
            missing=3, description="Number of convolutional blocks."
        )
    ...
    def _inception_module(self, input_tensor, stride=1, activation="linear"):
        ...
        if self.config.use_bottleneck and int(input_tensor.shape[-1]) > 1:
            input_inception = tf.keras.layers.Conv1D(
                filters=self.config.bottleneck_size,
                kernel_size=3,
                padding="CAUSAL",
        ...
        max_pool_1 = tf.keras.layers.MaxPool1D(
            pool_size=3, strides=self.config.n_strides, padding="SAME"
        )(input_tensor)
    ...
    def build(self, input_shape):
        ...
        output_layer = tf.keras.layers.Dense(
            self.config.nb_class, activation=self.config.output_activation
        )(gap_layer)
...
class TransformerCNN(BaseCustomTempnetsModel):
    ...
    class TransformerCNN(BaseCustomTempnetsModel._Schema):
        ...
        num_transformer_blocks = fields.Int(
            missing=4, description="Number of transformer blocks."
        )
        ff_dim = fields.Int(
            missing=4, description="Number of feed-forward neurons in point-wise CNN."
        )
        ...
        mlp_units = fields.List(
            fields.Int,
            missing=[64],
            description="Number of units for each layer in mlp.",
        )
        mlp_dropout = fields.Float(
            required=True,
            description="Keep probability used in dropout MLP layers.",
            example=0.5,
        )
    ...
    def transformer_encoder(self, inputs):
        ...
        x = tf.keras.layers.MultiHeadAttention(
            ...
            dropout=1 - self.config.keep_prob,
        )(x, x)
        ...
        x = tf.keras.layers.Conv1D(
            padding="SAME", filters=inputs.shape[-1], kernel_size=1
        )(x)
        return x, res
    ...
    def _embeddings(self, net):
        ...
        if self.config.emb_layer == "Flatten":
            ...
            net = tf.keras.layers.GlobalAveragePooling1D(
                name=name, data_format="channels_first"
            )(net)
        elif self.config.emb_layer == "GlobalMaxPooling1D":
    ...
    def build(self, inputs_shape):
        ...
        outputs = tf.keras.layers.Dense(
            self.config.n_classes, activation=self.config.output_activation
        )(x)
...
class CNNTaeSchema(BaseCustomTempnetsModel):
    ...
    def init_model(self):
        ...
        mlp4_tf.keras.layers = [
            pse_tae_tf.keras.layers.LinearLayer(out_dim) for out_dim in self.config.mlp4
        ]
        # Final layer (logits)
        mlp4_tf.keras.layers.append(
            pse_tae_tf.keras.layers.LinearLayer(
                1, batch_norm=False, activation="linear"
            )
        )
