...
from .transformer_encoder_layers import (
...
class PixelSetEncoder(tf.keras.layers.Layer):
    def __init__(self, mlp1=[10, 32, 64], mlp2=[64, 128], pooling="mean_std"):
        ...
        self.encoder = tf.keras.Sequential([self.mlp1, self.pooling, self.mlp2])
...
class MultiHeadAttention(tf.keras.layers.Layer):
    def __init__(self, n_head, d_k, name="multi_head_attention"):
        ...
        self.fc2 = tf.keras.Sequential([L.BatchNormalization(), L.Dense(d_k)])
...
class TemporalAttentionEncoder(tf.keras.layers.Layer):
    ...
    def call(self, x, training=None, mask=None):
        ...
        enc_output = self.attention_heads(
            enc_output, enc_output, enc_output, training=training, mask=mask
        )
...
class SummaryConcatenate(tf.keras.layers.Layer):
    ...
    def call(self, x, training=None, mask=None):
        layer_outputs = [
            layer(x, training=training, mask=mask) for layer in self.layers
        ]
