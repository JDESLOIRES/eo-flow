...
class BaseModelMultibranch(BaseModelCustomTraining):
        ...
        for x_dyn_batch_train, x_static_batch_train, y_batch_train in train_ds:
            with tf.GradientTape() as tape:
                if not tabular_data:
                    x_dyn_batch_train = [
                        x_dyn_batch_train[..., i]
                        for i in range(x_dyn_batch_train.shape[-1])
                    ]
                    y_preds, _ = self.call(
                        [x_dyn_batch_train, x_static_batch_train], training=True
                    )
                else:
                    x_train = self._reshape_array(
                        x_dyn_batch_train.numpy(), x_static_batch_train.numpy()
                    )
    ...
    def val_step_mb(self, val_ds, tabular_data=False):
        ...
        for x_dyn_batch_train, x_static_batch_train, y_batch_train in val_ds:
            if not tabular_data:
                ...
                y_preds, _ = self.call(
                    [x_dyn_batch_train, x_static_batch_train], training=False
                )
            else:
                x_train = self._reshape_array(
                    x_dyn_batch_train.numpy(), x_static_batch_train.numpy()
    ...
    def predict_mb(self, x_dyn_batch_train, x_static_batch_train, tabular_data):
        if not tabular_data:
            x_dyn_batch_train = [
                x_dyn_batch_train[..., i] for i in range(x_dyn_batch_train.shape[-1])
            ]
            y_preds, _ = self.call(
                [x_dyn_batch_train, x_static_batch_train], training=False
            )
    ...
    def fit_mb(
        ...
        **kwargs
        ...
        for epoch in range(num_epochs + 1):
            x_dyn_train_, x_static_train_, y_train_ = shuffle(
            ...
            train_ds = tf.data.Dataset.from_tensor_slices(
                (x_dyn_train_, x_static_train_, y_train_)
            ).batch(batch_size)
