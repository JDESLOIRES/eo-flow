...
class BaseModelSLLTraining(BaseModelCustomTraining):
    ...
    @tf.function
    def train_step_ssl(self, train_ds, f_map=True, lambda_=0.5):
        ...
        for (
            ...
            with tf.GradientTape() as tape:
                ...
                cost = self.loss(y_batch_train, y_preds) + lambda_ * self.loss(
                    orig, shift
                )
    ...
    def valstep_ssl(self, train_ds):
        ...
        self.metric.update_state(
            tf.reshape(y_batch_train[:, 0], tf.shape(y_preds)), y_preds
        )
        ...
        for epoch in range(num_epochs + 1):
            if unl_dataset.shape[0] > x_train.shape[0]:
                random_obs = np.random.choice(
                    unl_dataset.shape[0], size=x_train.shape[0], replace=False
                )
                x_unl = unl_dataset[
                    random_obs,
                ]
            else:
                unl_dataset = np.repeat(
                    unl_dataset, x_train.shape[0] // unl_dataset.shape[0], axis=0
                )
                num_missing = x_train.shape[0] - unl_dataset.shape[0]
                additional_obs = np.random.choice(
                    unl_dataset.shape[0], size=num_missing, replace=False
                )
            ...
            if shift_step:
                x_unl_shift, _ = data_augmentation(
                    x_unl,
            ...
            train_ds = tf.data.Dataset.from_tensor_slices(
                (x_train_, x_unl, x_unl_shift, y_train_)
            ).batch(batch_size)
