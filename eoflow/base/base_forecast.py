...
class BaseModelForecast(BaseModelCustomTraining):
    ...
    def fit_forecast(self,
        ...
        train_loss = []
        self._init_ssl_models(input_shape=x_is.shape[-1],
        ...
        for ep in range(num_epochs):
            ...
            train_ds = tf.data.Dataset.from_tensor_slices((x_is_, x_orig_, y_preds_)).batch(batch_size)

            for x_is_batch_train, x_orig_batch_train, y_batch_train in train_ds:
                self.forecast_step(xis=x_is_batch_train,
            ...
            print('Epoch ' + str(ep) + ' : ' + str(loss_epoch) + '; reconstruction ' + str(loss_reco) + '; task ' +str(loss_task)  )
    ...
    def fit_is(self,
        ...
        self._init_ssl_models(input_shape=x_is_train.shape[-1],
        ...
        test_ds = tf.data.Dataset.from_tensor_slices((x_is_test, y_test)).batch(batch_size)
        ...
        for epoch in range(num_epochs):
            ...
            x_is_train_, x_orig_train_, y_train_, y_preds_ = shuffle(x_is_train, x_orig_train, y_train, y_preds)
            train_ds = tf.data.Dataset.from_tensor_slices((x_is_train_, x_orig_train_, y_train_, y_preds_)).batch(batch_size)

            for x_is_batch_train, x_orig_batch_train, y_batch_train, y_preds_batch in train_ds:
                self.trainstep_is(xis=x_is_batch_train,
