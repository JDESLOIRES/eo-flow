...
# inherits frpm BaseModelAdapt normaly
...
class BaseModelAdaptV2:  # (BaseModelAdapt):
    ...
    @tf.function
    def trainstep_dann_v2(self, train_ds, lambda_=1.0):
        ...
        for Xs, ys, Xt, yt in train_ds:
            with tf.GradientTape(persistent=True) as gradients_task:
                if self.config.loss in ["gaussian", "laplacian"]:
                    ...
                    ys_pred, sigma_s = tf.reshape(ys_pred, tf.shape(ys)), tf.reshape(
                        sigma_s, tf.shape(ys)
                    )
                ...
                disc_loss = 0.5 * (
                    self.discriminator.loss(tf.ones_like(ys_disc), ys_disc)
                    + self.discriminator.loss(tf.zeros_like(yt_disc), yt_disc)
                )
                loss_dann = 0.5 * (
                    self.discriminator.loss(tf.zeros_like(ys_disc), ys_disc)
            ...
            grads_disc = gradients_task.gradient(
            ...
            # Update weights
            opt_op_task = self.optimizer.apply_gradients(
                zip(grads_task, self.trainable_variables)
            )
            disc_op_task = self.discriminator.optimizer.apply_gradients(
    ...
    def valstep_dann_v2(self, val_ds):
        for x_batch, y_batch in val_ds:
            if self.config.loss in ["gaussian", "laplacian"]:
                ...
                y_pred, sigma_ = tf.reshape(y_pred, tf.shape(y_batch)), tf.reshape(
                    sigma_, tf.shape(y_batch)
                )
    ...
    def fit_dann_v2(
        ...
        train_loss, val_loss, val_acc, test_loss, test_acc, disc_loss, task_loss = (
            [np.inf] if function == np.min else [-np.inf] for i in range(7)
        )
        ...
        self.discriminator.loss = tf.keras.losses.BinaryCrossentropy(
        ...
        val_ds = tf.data.Dataset.from_tensor_slices(
            (x_v.astype("float32"), y_v.astype("float32"))
        ).batch(batch_size)
        test_ds = tf.data.Dataset.from_tensor_slices(
            (x_t.astype("float32"), y_t.astype("float32"))
        ).batch(batch_size)
        ...
        # History of the training
        losses = dict(
            train_loss_results=train_loss[1:],
