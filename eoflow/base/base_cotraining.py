...
class BaseModelCoTraining(BaseModelCustomTraining):
    ...
    def _clone_model(self, x_train):
        ...
        model = tf.keras.Model(
            inputs=self.layers[0].input, outputs=Dense(units=1)(top_model)
        )
    ...
    def _get_lambda(factor, num_epochs, epoch):
        ...
        return tf.constant(
            factor * (2.0 / (1.0 + np.exp(-10.0 * p, dtype=np.float32)) - 1.0),
            dtype="float32",
        )
    ...
    def _init_model_pretrain(self, x, shift, noise):
        ...
        if noise and shift:
            model = tf.keras.Model(
                inputs=self.layers[0].input, outputs=[output_layer, output_layer_shift]
            )
        elif noise:
            model = tf.keras.Model(inputs=self.layers[0].input, outputs=output_layer)
        elif shift:
            model = tf.keras.Model(
                inputs=self.layers[0].input, outputs=output_layer_shift
            )
    ...
    def pretraining(
        ...
        for epoch in range(num_epochs):
            ...
            if shift:
                ts_masking, shift_arr, mask_sh = timeshift(
                    ts_masking, shift, proba=proba
                )
                shift_arr /= np.max(shift_arr)
            else:
                mask_sh, shift_arr = list(np.zeros(ts_masking.shape[0])), list(
                    np.zeros(ts_masking.shape[0]).astype(int)
                )

            train_ds = tf.data.Dataset.from_tensor_slices(
                (x, ts_masking, noise_added, mask, shift_arr, mask_sh, aux_vector_sh)
            )
                        ...
                        cost = tf.reduce_mean(
                            tf.multiply(
                                tf.math.squared_difference(
                                    tf.cast(noise_added_batch, "float32"), x_preds
                                ),
                                mask_batch,
                            )
                        ) + lambda_ * tf.reduce_mean(
            ...
            if epoch % 5 == 0:
                print(
                    "Epoch {0}: Train loss {1}".format(
                        str(epoch), str(self.loss_metric.result().numpy())
                    )
                )
    ...
    def _init_weights_pretrained(
        self, x, shift, noise, finetuning=0, set_fc_layers=False, model_directory="./"
    ):
        ...
        if (
            self.config.multioutput
            or self.config.loss in ["gaussian", "laplacian"]
            or not set_fc_layers
        ):
            return
        elif set_fc_layers:
            for i in range(
                self.config.nb_conv_stacks * 4 + 2, len(self.layers[0].layers)
            ):
                self.layers[0].layers[i].set_weights(model.layers[i].get_weights())

    def multitask_pretraining(
        self,
        train_dataset,
        val_dataset,
        test_dataset,
        ...
        for epoch in range(num_epochs):
            ...
            if not additional_target:
                ...
                ts_masking, shift_arr, mask_sh, = (
                    x_train_,
                    additional_target,
                    np.zeros((x_train_.shape[0]), dtype="float32") + 1,
                )
            shift_arr = (shift_arr - np.min(shift_arr)) / (
                np.max(shift_arr) - np.min(shift_arr)
            )

            train_ds = tf.data.Dataset.from_tensor_slices(
                (ts_masking, y_train_, shift_arr, mask_sh)
            )
            train_ds = train_ds.batch(batch_size)
            ...
            for (
                ...
                with tf.GradientTape() as tape:
                    ...
                    cost_unsupervised = tf.reduce_mean(
                        tf.multiply(self.loss(shift_batch, aux_preds), mask_sh_batch)
                    )
            ...
            if epoch % 5 == 0:
                ...
                print(
                    "Epoch {0}: Train loss {1}, Val loss {2}, Val acc {3}, Test loss {4}, Test acc {5}".format(
                        ...
                        str(round(test_loss_epoch, 4)),
                        str(round(test_acc_result, 4)),
                    )
                )
    ...
    def _cotrain_step(
        ...
        lambda_,
        n_forget,
        div_loss=tf.keras.losses.MeanSquaredError(
        ...
        for epoch in range(num_epochs + 1):
            ...
            for x_batch, y_batch in train_ds:  # tqdm
                with tf.GradientTape() as tape:
                    ...
                    y_preds_1, cost_1, jcor = self._cotrain_step(
                        model, x_batch, y_batch, y_preds_2, lambda_, forget_
                    )
                    cost_2 = self._cost_cotraining(
                        y_batch, y_preds_2, jcor, lambda_, forget
                    )
