...
class BaseSegmentationModel(BaseModelTraining):
    ...
    class _Schema(Schema):
        n_classes = fields.Int(
            required=True, description="Number of classes", example=2
        )
        ...
        prediction_visualization = fields.Bool(
            missing=False, description="Record prediction visualization summaries."
        )
        prediction_visualization_num = fields.Int(
            missing=5, description="Number of images used for prediction visualization."
        )
    ...
    def _prepare_class_weights(self):
        ...
        return np.array(
            [
                self.config.class_weights[iclass]
                if iclass in self.config.class_weights
                else 1.0
        ...
        * Otherwise the argument is passed to `compile` as is.
        ...
        if loss in segmentation_losses:
            loss = segmentation_losses[loss](
                from_logits=False, class_weights=class_weights
            )
                    ...
                    wrapped_metrics += [
                        CroppedMetric(
                            segmentation_metrics[metric](
                                top_k=1, class_id=class_id, name=f"{metric}_{class_id}"
                            )
                        )
        ...
        self.compile(
            optimizer=optimizer, loss=wrapped_loss, metrics=wrapped_metrics, **kwargs
        )
    ...
    def train_and_evaluate(
        ...
        **kwargs,
        ...
        if self.config.prediction_visualization:
            log_dir = os.path.join(model_directory, "logs", "predictions")
            visualization_callback = self._get_visualization_callback(
                val_dataset, log_dir
            )
            custom_callbacks.append(visualization_callback)
