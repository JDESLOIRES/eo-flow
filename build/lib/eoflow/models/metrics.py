...
class MeanIoU(InitializableMetric):
    ...
    def init_from_config(self, model_config=None):
        ...
        if model_config is not None and "n_classes" in model_config:
            self.metric = tf.keras.metrics.MeanIoU(
                num_classes=model_config["n_classes"]
            )
        else:
            print(
                "n_classes not found in model config or model config not provided. Using default max value."
            )
...
class CroppedMetric(tf.keras.metrics.Metric):
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        labels_crop = tf.image.resize_with_crop_or_pad(
            y_true, logits_shape[1], logits_shape[2]
        )
...
class MCCMetric(InitializableMetric):
    ...
    def init_from_config(self, model_config=None):
        ...
        if model_config is not None and "n_classes" in model_config:
            self.metric = tfa.metrics.MatthewsCorrelationCoefficient(
                num_classes=model_config["n_classes"]
            )
        else:
            print(
            ...
            self.metric = tfa.metrics.MatthewsCorrelationCoefficient(
                num_classes=self.default_n_classes
            )
...
class GeometricMetrics(InitializableMetric):
    ...
    def _segmentation_error(intersection_area: float, object_area: float) -> float:
        return 1.0 - intersection_area / object_area
    ...
    def _border_err(
        self, border_ref_edge: np.ndarray, border_meas_edge: np.ndarray
    ) -> float:
    ...
    def _fragmentation_err(self, r: int, reference_mask: np.ndarray) -> float:
        ...
        return (r - 1.0) / den if den > 0 else 0
    ...
    def __init__(
        self, pixel_size: int = 1, edge_func: Callable = None, **edge_func_params: Any
    ):
    ...
    def update_state(
        ...
        if not tf.executing_eagerly():
            warnings.warn(
                "Geometric metrics must be run with eager execution. If running as a compiled Keras model, "
        ...
        measurement = (
            measurement.numpy() if isinstance(reference, tf.Tensor) else measurement
        )
        ...
        for ref, meas in zip(reference, measurement):
            ...
            components_reference = set(np.unique(cc_reference)).difference(
                [background_value]
            )
            ...
            for component in components_reference:
                ...
                uniq, count = np.unique(
                    cc_measurement[
                        reference_mask & (cc_measurement != background_value)
                    ],
                    return_counts=True,
                )
                ref_area = np.sum(reference_mask)

                max_interecting_measurement = (
                    uniq[count.argmax()] if len(count) > 0 else background_value
                )
                meas_mask = cc_measurement == max_interecting_measurement
                meas_area = np.count_nonzero(
                    cc_measurement == max_interecting_measurement
                )
                intersection_area = count.max() if len(count) > 0 else 0

                self.oversegmentation_error.append(
                    self._segmentation_error(intersection_area, ref_area)
                )
                self.undersegmentation_error.append(
                    self._segmentation_error(intersection_area, meas_area)
                )
                border_ref_edge = ref_edges.squeeze() & reference_mask.squeeze()
                border_meas_edge = meas_edges.squeeze() & meas_mask.squeeze()

                self.border_error.append(
                    self._border_err(border_ref_edge, border_meas_edge)
                )
                self.fragmentation_error.append(
                    self._fragmentation_err(len(uniq), reference_mask)
                )
    ...
    def result(self) -> List[float]:
        ...
        return [
            self.get_oversegmentation_error(),
            self.get_undersegmentation_error(),
            self.get_border_error(),
            self.get_fragmentation_error(),
        ]
