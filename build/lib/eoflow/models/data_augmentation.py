    ...
    for i in range(ts_masking.shape[0]):
        for j in range(ts_masking.shape[1]):
            ...
            if prob < proba:
                ...
                mask[i, j] = 1.0
                if prob < 0.5:
                    ts_masking[i, j, :] += np.random.uniform(
                        low=-value, high=0, size=(x_batch.shape[2])
                    )
                else:
                    ts_masking[i, j, :] += np.random.uniform(
                        low=0, high=value, size=(x_batch.shape[2])
                    )
