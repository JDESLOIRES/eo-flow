...
cosine_sim_1d = tf.keras.losses.CosineSimilarity(
...
class BaseModelSelfTraining(BaseModelCustomTraining):
    ...
    def _init_ssl_models(self, input_shape, output_shape, add_layer=False):
        ...
        linear_layer1 = Dense(
            self.layers[0].layers[0].input.shape[-1], activation=None, name="linear_1"
        )(latent)
        z = tf.keras.layers.LeakyReLU()(linear_layer1)
        z = Dense(
            self.layers[0].layers[0].input.shape[-1], activation=None, name="linear_2"
        )(z)
    ...
    def subset_generator(
        ...
        permuted_order = (
            np.random.permutation(n_subsets) if mode == "train" else range(n_subsets)
        )
    ...
    def pretrain_step(self, concatenated_subsets_list, x_orig, temperature, rho=0):
        ...
        for el in concatenated_subsets_list:
            ...
            with tf.GradientTape(persistent=True) as tape:
                ...
                loss_reconstruction += tf.reduce_mean(
                    (self.loss(x_orig, x_reco_is) + self.loss(x_orig, x_reco_js)) / 2
                )
                ...
                if rho:
                    ...
                    kl = (
                        self.kl_divergence(rho, rho_is + 1e-10)
                        + self.kl_divergence(rho, rho_js + 1e-10)
                    ) / 2
                    kl = tf.reduce_sum(kl)
        ...
        self.projection.optimizer.apply_gradients(
            zip(gradients, self.projection.trainable_variables)
        )
    ...
    def _get_weights(
        self, x_dyn_train, x_stat_train, y_train, n_subsets, overlap, swap=False
    ):

        x_tilde = self.subset_generator(
            x_dyn_train,
            x_stat_train,
            n_subsets=n_subsets,
            overlap=overlap,
        ...
        w_loss = [
            tf.reduce_mean(self.loss(y_train, self.task(h))).numpy() for h in h_tilde
        ]
    ...
    def subtab_train_step(self, x_tilde, y_batch_train, kd, lambda_=1):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            w_loss = [
                tf.reduce_mean(self.loss(y_batch_train, self.task(h))).numpy()
                for h in h_tilde
            ]
            w_loss /= np.sum(w_loss)
            w_loss = 1 / w_loss
    ...
    def subtab_pred_step(
        self, x_test_, model_directory, n_subsets=3, overlap=0.75, w_loss=None
    ):
        ...
        x_tilde = self.subset_generator(
            x_test, n_subsets=n_subsets, overlap=overlap, mode="test", swap=False
        )
    ...
    def fit_pretrain(
        ...
        if x_static is not None:
            ...
            self._init_ssl_models(
                input_shape=stop_idx,
                output_shape=np.concatenate([x_dynamic, x_static], axis=1).shape[-1],
            )
        else:
            stop_idx = n_column_subset + n_overlap
            self._init_ssl_models(
                input_shape=stop_idx, output_shape=x_dynamic.shape[-1]
            )

        if model_kd is None:
            y_preds = np.zeros((x_dynamic.shape[0], 1))
        elif model_kd.config.adaptative:
            y_preds, h_latent = model_kd.predict(
                np.concatenate([x_dynamic, x_static], axis=1)
            )
        else:
        ...
        for ep in range(num_epochs):
            ...
            train_ds = tf.data.Dataset.from_tensor_slices(
                (x_dynamic_, x_static_, y_preds_)
            ).batch(batch_size)
            for x_dyn_batch_train, x_stat_batch_train, y_batch_train in train_ds:
                x_tilde_list = self.subset_generator(
                    x_dynamic=x_dyn_batch_train.numpy(),
                ...
                concatenated_subsets_list = self.get_combinations_of_subsets(
                    x_tilde_list
                )
                self.pretrain_step(
                    concatenated_subsets_list,
                    x_orig=np.concatenate(
                        [x_dyn_batch_train.numpy(), x_stat_batch_train.numpy()], axis=1
                    ),
    ...
    def fit_supervised(
        ...
        if model_kd is None:
            ...
            y_preds, h_latent = model_kd.predict(
                np.concatenate([x_dyn_train, x_stat_train], axis=1)
            )
        else:
            y_preds = model_kd.predict(
                np.concatenate([x_dyn_train, x_stat_train], axis=1)
            )
        ...
        val_ds = tf.data.Dataset.from_tensor_slices(
            (x_dyn_val, x_stat_val, y_val)
        ).batch(batch_size)
        test_ds = tf.data.Dataset.from_tensor_slices(
            (x_dyn_test, x_stat_test, y_test)
        ).batch(batch_size)
        ...
        for epoch in range(num_epochs):
            ...
            x_dyn_train_, x_stat_train_, y_train_, y_preds_ = shuffle(
                x_dyn_train, x_stat_train, y_train, y_preds
            )
            train_ds = tf.data.Dataset.from_tensor_slices(
                (x_dyn_train_, x_stat_train_, y_train_, y_preds_)
            ).batch(batch_size)
            ...
            for (
                ...
                x_tilde_train = self.subset_generator(
                    x_dynamic=x_dyn_batch_train.numpy(),
                    x_static=x_stat_batch_train.numpy(),
                    n_subsets=n_subsets,
                    overlap=overlap,
            ...
            if epoch % save_steps == 0:
                wait += 1
                w_h = self._get_weights(
                    x_dyn_train_, x_stat_train_, y_train_, n_subsets, overlap
                )
                for x_dyn_batch_train, x_stat_batch_train, y_batch_train in val_ds:
                    x_tilde_val = self.subset_generator(
                        x_dynamic=x_dyn_batch_train.numpy(),
                        x_static=x_stat_batch_train.numpy(),
                        n_subsets=n_subsets,
                        overlap=overlap,
                ...
                for x_dyn_batch_train, x_stat_batch_train, y_batch_train in test_ds:
                    x_tilde_test = self.subset_generator(
                        x_dynamic=x_dyn_batch_train.numpy(),
                        x_static=x_stat_batch_train.numpy(),
                        n_subsets=n_subsets,
                        overlap=overlap,
