...
class BaseModelCustomTraining(tf.keras.Model, Configurable):
    ...
    @tf.function
    def train_step(self, train_ds, n_forget=0, size_batch=8, lambda_=1):

        for x_batch_train, y_batch_train, y_batch_aux in train_ds:
            if self.config.multibranch:
                x_batch_train = [
                    x_batch_train[..., i] for i in range(x_batch_train.shape[-1])
                ]
    ...
    def val_step(self, val_ds, lambda_=1):
        for x_batch_train, y_batch_train, y_batch_aux in val_ds:
            ...
            if self.config.multioutput:
                ...
                y_true, y_aux_true = y_batch_train, y_batch_aux
                cost = self.loss(y_true, y_preds) + lambda_ * self.loss(
                    y_aux_true, y_aux
                )
            elif self.config.loss in ["gaussian", "laplacian"]:
            ...
            self.metric.update_state(
                y_batch_train.numpy().flatten(), y_preds.numpy().flatten()
            )

    def _reduce_lr_on_plateau(self, patience=30, factor=0.1, reduce_lin=False):

        return CustomReduceLRoP(
            patience=patience,
            factor=factor,
            verbose=1,
    ...
    def fit(
        ...
        if self.config.multioutput:
            ...
            y_train_aux, y_val_aux, y_test_aux = (
        ...
        if not self.config.multibranch:
            ...
            shapes = [
                tf.zeros(list((x_dyn_train.shape[0], x_dyn_train.shape[1], 1)))
        ...
        val_ds = tf.data.Dataset.from_tensor_slices((x_val, y_val, y_val_aux)).batch(
            batch_size
        )
        test_ds = tf.data.Dataset.from_tensor_slices(
            (x_test, y_test, y_test_aux)
        ).batch(batch_size)
        ...
        for epoch in range(num_epochs + 1):
            ...
            train_ds = tf.data.Dataset.from_tensor_slices(
                (x_train_, y_train_, y_train_aux_)
            ).batch(batch_size)
