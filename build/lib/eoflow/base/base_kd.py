...
class BaseModelKD(BaseModelCustomTraining):
    ...
    def _init_dataset_training(x_s, y_s, x_t, y_t, batch_size):
        ...
        return tf.data.Dataset.from_tensor_slices((x_s, y_s, x_t, y_t)).batch(
            batch_size
        )

    @tf.function
    def trainstep_teacher(self, teacher_model, train_ds):
        ...
        for Xs, ys, _, _ in train_ds:
            ...
            grads_task = gradients_task.gradient(
                cost_teacher, teacher_model.trainable_variables
            )
            cost_teacher += sum(teacher_model.losses)

            # Update weights
            teacher_model.optimizer.apply_gradients(
                zip(grads_task, teacher_model.trainable_variables)
            )
    ...
    def trainstep_student(
        ...
        for Xs, ys, Xt, yt in train_ds:
            with tf.GradientTape() as gradients_task:
                ...
                cost_student = self.loss(yt, yt_pred) + gamma_ * consis_loss(
                    ys_pred, yt_pred
                )
                if lambda_ > 0:
