...
class TrainTask(BaseTask):
    class TrainTaskConfig(Schema):
        ...
        save_steps = fields.Int(
            missing=100,
            description="Number of training steps between model checkpoints.",
        )
        summary_steps = fields.Int(
            missing="epoch",
            description="Number of training steps between recording summaries.",
        )
...
class TrainAndEvaluateTask(BaseTask):
    class TrainAndEvaluateTask(Schema):
        ...
        train_input_config = fields.Nested(
            nested=ObjectConfiguration,
            required=True,
            description="Input type and configuration for training.",
        )
        val_input_config = fields.Nested(
            nested=ObjectConfiguration,
            required=True,
