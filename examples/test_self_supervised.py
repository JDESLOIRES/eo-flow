...
def load_gdd_data(
    ...
    return (
        np.concatenate([training_x_s1, training_x_s2], axis=1),
        training_x_s3,
        np.concatenate([val_x_s1, val_x_s2], axis=1),
        val_x_s3,
        np.concatenate([test_x_s1, test_x_s2], axis=1),
        test_x_s3,
        training_y,
        val_y,
        test_y,
...
model_compiled.encoder.load_weights(
    os.path.join(
        "/home/johann/Documents/Experiments SUBTAB/SSL_ENC_128_NOMASK/" + str(year),
        "encoder_last_model",
    )
)
...
year
h, z_, x_reco, task = model_compiled.forward(
    np.concatenate([test_x_dyn, test_x_static], axis=1), training=False
)
...
model_compiled.fit(
    ...
    batch_size=12,
    num_epochs=200,
    model_directory="/home/johann/Documents/Experiments SUBTAB/SUBTAB_ENC_NODP_SWAP_SPARSE/"
    + str(year),
    save_steps=5,
    patience=0,
)
