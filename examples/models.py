...
class ExampleModel(BaseModelTraining):
    ...
    class _Schema(Schema):
        output_size = fields.Int(
            required=True, description="Output size of the model", example=10
        )
        hidden_units = fields.Int(
            missing=512, description="Number of hidden units", example=512
        )
        learning_rate = fields.Float(
            missing=0.01, description="Learning rate for Adam optimizer", example=0.01
        )
