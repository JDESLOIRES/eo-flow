...
class TestLosses(unittest.TestCase):
    def test_shapes(self):
        for loss_fn in [
            CategoricalFocalLoss(from_logits=True),
    ...
    def test_focal_loss_values(self):
        ...
        for loss_fn in [
            CategoricalFocalLoss(from_logits=False),
            ...
            val3 = loss_fn(
                y_true3, y_pred
            ).numpy()  # Should be in between (half are correct)
    ...
    def test_jaccard_loss(self):
        ...
        loss_fn = JaccardDistanceLoss(
            from_logits=False, smooth=1, class_weights=np.array([0, 1, 1])
        )
    ...
    def test_tanimoto_loss(self):
        ...
        self.assertEqual(
            TanimotoDistanceLoss(from_logits=False)(y_true, y_true).numpy(), 0.0
        )
        self.assertEqual(
            TanimotoDistanceLoss(from_logits=False)(y_pred, y_pred).numpy(), 0.0
        )
        self.assertAlmostEqual(
            TanimotoDistanceLoss(from_logits=False)(y_true, y_pred).numpy(), 1.25, 5
        )
        self.assertAlmostEqual(
            TanimotoDistanceLoss(from_logits=False, normalise=True)(
                y_true, y_pred
            ).numpy(),
            1.2460148,
            5,
        ...
        self.assertAlmostEqual(
            TanimotoDistanceLoss(from_logits=False, class_weights=np.array([1, 0]))(
                y_true, y_pred
            ).numpy(),
            0.25,
            5,
